...
- Shell prompt: Starship with a two-line format (`home/shell/starship.nix`). The icons come from the built-in `Symbols Nerd Font Mono` of WezTerm, scaled to 1.4 in `home/programs/wezterm.nix`. WezTerm lets a square icon overflow into the cell after it only when that cell holds a space with the same attributes, so the OS icon stays unstyled (`os.style = "none"`); a styled icon is squeezed into one cell and renders smaller than the text.
