...
in
{
  programs.starship = {
    ...
    settings = {
      ...
      # style, so it takes the default foreground like the rest of the
      # line, and the format writes an unstyled space after it for the
      # width of the larger symbol font. See home/programs/wezterm.nix for
      # the overflow rule.
      os = {
        disabled = false;
        style = "none";
      ...
      # than the rest of the prompt. The symbol carries the two spaces that
      # the larger symbol font needs.
      nix_shell = {
        heuristic = true;
      ...
      # unit. The symbol carries two spaces for the width of the larger
      # symbol font; one space left the glyph on top of the time. The style
      # is explicit: the scheme maps the plain "yellow" to an orange
