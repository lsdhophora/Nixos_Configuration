...
(defun cph-http-request (body)
  "Send a POST request with BODY to the local server, return the raw response."
  (let* ((buf (generate-new-buffer " *http-resp*"))
         ...
         (head (concat "POST / HTTP/1.1\r\nHost: 127.0.0.1\r\n"
                       "Content-Type: application/json\r\n"
                       "Content-Length: "
                       (number-to-string (string-bytes body))
                       "\r\n\r\n")))
    (process-send-string client (concat head body))
    (let ((deadline (+ (float-time) 5)) resp)
      (while (and (< (float-time) deadline)
                  (not (string-match-p "\"status\""
...
;; --- start the server, send a problem, verify everything ---
(cph-server-start)
