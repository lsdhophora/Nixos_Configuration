...
import { formatDurationShort, parseExpiresIn, parseLoopScheduleArgs, parseRemindScheduleArgs } from "./scheduling";
	...
	pi.registerCommand("loop", {
		...
		handler: async (args, ctx) => {
			...
			if (parsed.recurring.mode === "cron") {
				const expiresInMs = parseExpiresIn(undefined);
				const task = runtime.addRecurringCronTask(
					parsed.prompt,
					parsed.recurring.cronExpression,
					parsed.recurring.timezone,
					expiresInMs,
				);
				if (!task) {
				...
				ctx.ui.notify(`Scheduled cron ${task.cronExpression}${tz} (id: ${task.id}). ${expiryText(expiresInMs)}`, "info");
				if (parsed.recurring.note) ctx.ui.notify(parsed.recurring.note, "info");
				return;
			}

			const expiresInMs = parseExpiresIn(undefined);
			const task = runtime.addRecurringIntervalTask(parsed.prompt, parsed.recurring.durationMs, expiresInMs);
			ctx.ui.notify(
				`Scheduled every ${formatDurationShort(parsed.recurring.durationMs)} (id: ${task.id}). ${expiryText(expiresInMs)}`,
				"info",
			);
			if (parsed.recurring.note) ctx.ui.notify(parsed.recurring.note, "info");
		},
