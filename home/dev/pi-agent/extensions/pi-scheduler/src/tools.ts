...
import { formatDurationShort, parseExpiresIn, validateSchedulePromptAddInput } from "./scheduling";
	...
	cron: Type.Optional(
		Type.String({
			description:
				...
				"Optionally prefix with TZ=<IANA zone> (e.g. 'TZ=America/Los_Angeles 0 0 * * *').",
		}),
	),
	timezone: Type.Optional(
		Type.String({
			description:
				"IANA timezone for the cron expression (e.g. 'America/Los_Angeles'). " +
				"When omitted, the cron runs in the machine's local timezone. Ignored for interval/once tasks.",
		}),
	),
	expiresIn: Type.Optional(
		Type.String({
			description:
				"How long a recurring task stays active before auto-delete, e.g. '3d', '1w', '30d'. " +
				"Use 'never' / 'forever' / '0' for a task that never expires. Defaults to '3d'.",
		}),
	),
...
function expiryText(expiresInMs: number | undefined): string {
	if (expiresInMs === undefined) return "Never expires.";
	return `Expires in ${formatDurationShort(expiresInMs)}.`;
}
		...
		execute: async (
			...
			if (action === "add") {
				...
				if (validated.plan.mode === "cron") {
					const expiresInMs = parseExpiresIn(params.expiresIn);
					const task = runtime.addRecurringCronTask(
						prompt,
						validated.plan.cronExpression,
						validated.plan.timezone,
						expiresInMs,
					);
					if (!task) {
						return {
					...
					return {
						content: [
							{
								type: "text",
								text: `Recurring cron task scheduled (id: ${task.id}) with '${task.cronExpression}'${tz}. ${expiryText(expiresInMs)}${
									validated.plan.note ? ` ${validated.plan.note}` : ""
								}`,
							},
						],
				...
				const expiresInMs = parseExpiresIn(params.expiresIn);
				const task = runtime.addRecurringIntervalTask(prompt, validated.plan.durationMs, expiresInMs);
				return {
					content: [
						{
							type: "text",
							text: `Recurring task scheduled (id: ${task.id}) every ${formatDurationShort(validated.plan.durationMs)}. ${expiryText(expiresInMs)}${
								validated.plan.note ? ` ${validated.plan.note}` : ""
							}`,
						},
					],
