...
import { DEFAULT_LOOP_INTERVAL, FIFTEEN_MINUTES, ONE_MINUTE } from "./constants";
import {
...
export class SchedulerRuntime {
	...
	addRecurringIntervalTask(prompt: string, intervalMs: number, expiresInMs?: number): ScheduleTask {
	...
	addRecurringCronTask(prompt: string, cronExpression: string, timezone?: string, expiresInMs?: number): ScheduleTask | undefined {
		const id = this.createId();
		const createdAt = Date.now();
		const nextRunAt = computeNextCronRunAt(cronExpression, createdAt, timezone);
		...
		while (true) {
			...
			if (action.startsWith("⏱")) {
				...
				if (task.kind === "recurring") {
					...
					const nextRunAt = computeNextCronRunAt(normalizedCron.expression, Date.now(), task.timezone);
					if (!nextRunAt) {
	...
	private dispatchTask(task: ScheduleTask) {
		...
		if (task.cronExpression) {
			const next = computeNextCronRunAt(task.cronExpression, now + 1_000, task.timezone);
			if (!next) {
				this.tasks.delete(task.id);
				this.persistTasks();
				this.updateStatus();
		...
		try {
			...
			for (const task of list) {
				...
				const normalized: ScheduleTask = {
					...
					expiresAt: task.expiresAt === null ? undefined : task.expiresAt,
				};
	...
	private persistTasks() {
		...
		try {
			...
			const store: SchedulerStore = {
				version: 1,
				tasks: this.getSortedTasks().map((task) => ({ ...task, expiresAt: task.expiresAt ?? null })),
			};
