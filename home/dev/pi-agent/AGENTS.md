...
3. **Exa gate** — An exa tool call requires the `{exa}` prefix at the start of the user message. See the `exa-search` skill for the verification steps. Never bypass the gate, for example with curl.
