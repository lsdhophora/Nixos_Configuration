...
4. CLI 输出无需再跑任何对齐工具——布局由 elkjs 自动计算，CJK 全角宽度由 CLI 内部处理。
...
| 含 →/▼/◇ 的行凸出边框 | 编辑器字体（Iosevka）把箭头/几何/符号按 2 格渲染 | 已内置：CLI 将符号区（U+2190-21FF、U+25A0-27BF 等）按 2 格计算；无需手工处理 |
